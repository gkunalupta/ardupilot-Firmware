...
bool SPIDevice::acquire_bus(bool set, bool skip_cs)
{
    ...
    if (!set && cs_forced) {
        ...
        bus.spicfg.end_cb = nullptr;    //Callback is null
        bus.spicfg.ssport = PAL_PORT(device_desc.pal_line);  // SPI_PAD_Mode is selected: Specifying port of NSS pin
        bus.spicfg.sspad = PAL_PAD(device_desc.pal_line);  // SPI_PAD_MODE is selected : Specyfing pin of the port for NSS Pin
#if defined(STM32H7)
